...
#ifndef TRITON_ADAPTER_MATH_CONVERTER_H
#define TRITON_ADAPTER_MATH_CONVERTER_H
...
public:
    ...
    matchAndRewrite(MathOp op, typename MathOp::Adaptor adaptor,
      ...
      auto binOp = rewriter.create<HivmOp>(
          loc,
          TypeRange{},
          ValueRange{lhs, rhs},
          ValueRange{result}
...
public:
    using OpConversionPattern<MathOp>::OpConversionPattern;
    LogicalResult matchAndRewrite(MathOp op, PatternRewriter &rewriter) const override {
    }
};
...
public:
    ...
    LogicalResult matchAndRewrite(MathOp op, typename MathOp::Adaptor adaptor,
        ...
        auto initCValue = rewriter.create<arith::ConstantIntOp>(loc,
            /*value*/ initC, /*width*/ 1);
        auto newOp = rewriter.create<hivm::MmadL1Op>(
            ...
            adaptor.getEnableHf32() ? rewriter.getUnitAttr() : UnitAttr{}// enable hf32 mode
        );
